...
public class DefaultSmppClient implements SmppClient {
     ...
     * Executors.newCachedDaemonThreadPool() is recommended. The max threads
     * will never grow more than expectedSessions if NIO sockets are used.
     * @param expectedSessions The max number of concurrent sessions expected to
     * be active at any time. This number controls the max number of worker
     * threads that the underlying Netty library will use. If processing occurs
     * in a sessionHandler (a blocking op), be <b>VERY</b> careful setting this
     * to the correct number of concurrent sessions you expect.
     */
     ...
     * in a sessionHandler (a blocking op), be <b>VERY</b> careful setting this
     * to the correct number of concurrent sessions you expect.
     * @param monitorExecutor The scheduled executor that all sessions will
     * share to monitor themselves and expire requests. If null monitoring will
     * be disabled.
     */
